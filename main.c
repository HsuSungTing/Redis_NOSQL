...
int main() {
    while(1){
        int command;
        printf("Enter 1 to use Redis string, Enter 2 to use Redis Redis list, Enter 3 to use Redis sorted set, Enter 4 to use Redis hash\n");
        scanf("%d",&command);
        if(command==1)handle_input_HW1();
        else if(command==2)handle_input_HW3();
        else if(command==3)handle_input();
        else if(command==4)handle_input_HW6();
